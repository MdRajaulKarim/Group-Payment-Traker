...
function normalize(raw) {
  ...
  const transactions = (Array.isArray(raw.transactions) ? raw.transactions : [])
    ...
    .map(t => {
      ...
      if (!paidBy && state.members.length) paidBy = state.members[0].id;
...
function fillPayerSelect(sel, txn) {
  sel.innerHTML = '';
  // Change to list all active members (or the historical payer if removed)
  const ids = state.members.filter(m => m.active || m.id === txn.paidBy).map(m => m.id);
  ...
  if (!ids.includes(txn.paidBy)) txn.paidBy = ids[0] || null;
...
function renderRow(txn) {
  ...
  const methodSel = el('select', {
    class: 'cell-input', 'aria-label': 'Payment method',
  ...
  const payerSel = el('select', {
    class: 'cell-input', 'aria-label': 'Who paid',
...
function keepPayerValid(txn) {
  if (!txn.paidBy && state.members.length > 0) txn.paidBy = state.members[0].id;
}
...
function calculate() {
  // 1. Sort chronologically right before calculating
  state.transactions.sort((a, b) => new Date(a.date) - new Date(b.date) || a.createdAt - b.createdAt);
  save();
  renderTransactions();
...
function mergeData(file) {
  ...
  reader.onload = () => {
    const obj = safeParse(reader.result);
    if (!obj) { showToast('Invalid file format.', null, 4000); return; }
    ...
    // 1. MERGE MEMBERS (Map duplicate names to existing local IDs)
    const existingMemberIds = new Set(state.members.map(m => m.id));
    const existingMemberNames = new Set(state.members.map(m => m.name.toLowerCase()));
    const idMap = {}; // Maps imported IDs to local IDs

    normalized.members.forEach(m => {
      if (existingMemberIds.has(m.id)) {
        idMap[m.id] = m.id; // Exact match
      } else if (existingMemberNames.has(m.name.toLowerCase())) {
        // Name exists but ID is different. Link them together.
        const existingLocal = state.members.find(x => x.name.toLowerCase() === m.name.toLowerCase());
        idMap[m.id] = existingLocal.id;
      } else {
        // Brand new person
    ...
    // 2. MERGE EXPENSES (Skip exact duplicates)
    const existingTxnIds = new Set(state.transactions.map(t => t.id));
    // Create a fingerprint of Date + Amount + Title to catch duplicates
    const getFingerprint = t => `${t.date}_${t.amountPaise}_${t.title.toLowerCase().trim()}`;
    const existingFingerprints = new Set(state.transactions.map(getFingerprint));

    normalized.transactions.forEach(t => {
      // Update payer and split participants to use the mapped IDs
      t.paidBy = idMap[t.paidBy] || t.paidBy;

      // ADDED .filter(Boolean) to clean out any broken/deleted members
      t.participants = t.participants.map(id => idMap[id] || id).filter(Boolean);

      const fingerprint = getFingerprint(t);

      if (existingTxnIds.has(t.id) || existingFingerprints.has(fingerprint)) {
        skippedTxns++;
      } else {
        t.id = uid(); // <-- ADDED: Give it a fresh collision-proof ID
    ...
    // 3. Auto-sort chronologically
    state.transactions.sort((a, b) => new Date(a.date) - new Date(b.date) || a.createdAt - b.createdAt);

    save();
    renderAll();

    // 4. Notify the user of the exact results
    if (addedMembers === 0 && addedTxns === 0) {
      showToast(`Merge skipped: Found ${skippedTxns} duplicate payment(s).`, null, 5000);
    } else {
      showToast(`Merged! Added ${addedMembers} new member(s) & ${addedTxns} expense(s). Skipped ${skippedTxns} duplicates.`, null, 6000);
    }
  };
  reader.readAsText(file);
}
...
function doClear() {
  const prev = JSON.parse(JSON.stringify(state)); // Take a snapshot for undo
...
function doClearExpenses() {
  // Wipes expenses, but keeps state.members intact
  state.transactions = [];
  save();
  settlementRevealed = false;
  settlementEl.hidden = true;
...
function init() {
  ...
  $('clearExpenses').addEventListener('click', doClearExpenses);
  ...
  $('mergeFile').addEventListener('change', e => { mergeData(e.target.files[0]); e.target.value = ''; });
