...
5. **Copy**, **Download .txt**, or **Share on WhatsApp**. **Clear all** resets.
...
`group-payments-backup-<date>.json` file; *Import data* loads one back (it
replaces the current data and can be undone from the toast). Use it to move a
trip between browsers or devices, or just to keep a safe copy — the settlement
`.txt`/WhatsApp exports are summaries, this is the real data.
